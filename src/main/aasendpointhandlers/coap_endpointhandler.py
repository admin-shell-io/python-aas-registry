...
try:
    from aasendpointhandlers.coap_endpointresources import RetrieveMessageCoap, HandleConnectProtocolCoap, \
        HandleConnectCoap
except ImportError:
...
class AASEndPointHandler(AASEndPointHandler):
    ...
    def configure(self):
        ...
        self.ipaddressComdrv = self.pyAAS.lia_env_variable["LIA_AAS_COAP_HOST_EXTERN"]
    ...
    def run(self):
        ...
        asyncio.Task(
            aiocoap.Context.create_server_context(self.root, bind=(self.ipaddressComdrv, int(self.portComdrv))))
        self.pyAAS.service_logger.info("COAP namespaces are started")
        self.pyAAS.service_logger.info("COAP REST API namespaces are configured========================================")
