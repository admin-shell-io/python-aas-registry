...
class AASEndPointHandler(AASEndPointHandler):
    ...
    def dispatchMessage(self, send_Message): 
        try:
            targetHeader = {"content-type": "application/json","User-Agent":"VWS Registry Agent "+str(datetime.utcnow().isoformat(sep=' ', timespec='microseconds'))[17:]}
    ...
    def sendBroadCatMessage(self,send_Message,key):
        try:
            requests.post(url= self.pyAAS.httpEndPointsDict[key], data=json.dumps(send_Message), headers=self.targetHeader)
